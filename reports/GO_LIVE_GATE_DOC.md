...
**Current scope note:** Until the Apr 14 review is complete, this document should be interpreted against the current frozen paper lane only:
- CCI Trend ETHUSDT 4h
- Donchian Trend ETHUSDT 4h

It should not be read as blanket approval for every strategy that appears in any manifest, shortlist, or research report.
...
## Apr 14 Decision Rule

The Apr 14 verdict must be one of:
- `NO_GO`
- `PAPER_ONLY`
- `READY_FOR_TINY_CAPITAL`

That verdict must apply only to:
- `CCI Trend` on `ETHUSDT 4h`
- `Donchian Trend` on `ETHUSDT 4h`

The verdict must not automatically include:
- research-lane winners such as G23-G36
- separate AVAX/SUI/DOT/LDO/BTC candidates
- any higher-drawdown strategies approved under a different evidence path
